...
module.exports = function(schema) {
	...
	return {
		save: saveTask,
		clean: cleanTask,
		status: Action.prototype.STATUS
	};
};
...
function saveTask(context, config, job, done) {
	// Register action.
	var saveAction = new Action('Save artifact to repository', context, done);
...
function cleanTask(context, config, job, done) {
	// Register action.
	var cleanAction = new Action('Clean old artifacts', context, done);
	...
	if (config.maxBuilds > 0) {
		...
		done(cleanAction.STATUS.SUCCESS);
	}
}
